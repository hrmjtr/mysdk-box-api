...
| クラス + メソッド               | 構造体 + レシーバ付き関数           | `func (c *Client) Space()` |
...
| `nil` になりうる値              | ポインタ型(`*User`)               | 値型はゼロ値になり nil を表せない |
...
type Issue struct {
	...
	Assignee    *User    `json:"assignee"`     // null になりうる → ポインタ
	Status      Status   `json:"status"`
	...
}
```

- バッククォート内の `json:"..."` が **構造体タグ**。JSON のキー(camelCase)と
  Go のフィールド名(PascalCase)の対応を宣言する。
- JSON に含まれる未定義フィールドは黙って捨てられる(エラーにならない)。
  「必要なフィールドだけ定義する」という本リポジトリの方針と相性がよい。
- `assignee` は API 仕様上 null になりうるため `*User`(ポインタ)にする。
  値型 `User` のままだと null がゼロ値の User になり、null と区別できない。
  `description` は「null なら空文字でよい」と割り切って string にしている。
  ここは設計判断で、厳密に区別したいなら `*string` にする。

### 2. エラー設計:例外がない世界(errors.go)
...
### 3. ジェネリクスで get を 1 つにする(client.go)
...
- `T` には構造体(`Space`)もスライス(`[]Project`)も入る。
- メソッドは型パラメータを持てないという Go の制約があるため、
  `get` はメソッドではなくパッケージ関数にしてある。

### 4. レスポンス判定:パースエラーと想定外の区別が Go では自然にできる
...
静的型付けの恩恵で、「配列を期待したのにオブジェクトが来た」まで検出できる
(Ruby 版はスカラー値しか検出できない)。

- `http.Client.Get` は Ruby の `Net::HTTP` と同じく **非 2xx でも err を返さない**。
  err が返るのは通信自体の失敗のみ。ステータス判定は自分で書く。
- `defer res.Body.Close()` は Ruby のブロック付き open と同じ「確実に閉じる」慣用句。

### 5. 利用側のエラー分岐(example/main.go)
...
2. `models.go`: api.md のデータモデルを構造体 + json タグで定義
   (null になりうるフィールドはポインタに)
3. `errors.go`: 番兵 1 つ + エラー型 3 つ、`Error()` と `Unwrap()` を実装
4. `client.go`: `get[T any]` にステータス → 空 Body → Unmarshal の判定を実装、
...
- **context 対応**: 実務の Go では `Space(ctx context.Context)` のように
  第一引数で ctx を受けてキャンセル可能にするのが標準的。
  `http.NewRequestWithContext` を使う形に `get` を書き換える。
- **日時のパース**: `Created` を string でなく `time.Time` にすると
